...
class OnlineGame(ConnectionListener):
    ...
    def __init__(self):
        ...
        # Create the players
        self.players = []
        self.players.append(Player(pygame.image.load("res/images/naruto-sprites/naruto-fall1.png")))
        self.players.append(Player(pygame.image.load("res/images/sasuke-sprites/sasuke-fall1.png")))
    ...
    def check_keys(self):
        ...
        if keys[K_UP]:
            ...
            connection.Send({"action": "move", "x": 0, "y": -self.velocity, "player": self.player, "gameID": self.gameID})
        if keys[K_DOWN]:
            self.players[self.player].rect.y += self.velocity
            connection.Send({"action": "move", "x": 0, "y": self.velocity, "player": self.player, "gameID": self.gameID})
        if keys[K_LEFT]:
            self.players[self.player].rect.x -= self.velocity
            connection.Send({"action": "move", "x": -self.velocity, "y": 0, "player": self.player, "gameID": self.gameID})
        if keys[K_RIGHT]:
            self.players[self.player].rect.x += self.velocity
            connection.Send({"action": "move", "x": self.velocity, "y": 0, "player": self.player, "gameID": self.gameID})
